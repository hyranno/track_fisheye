...
from compute_shader.pairing_convolutions import PairingConvolutionShader
...
class MarkerPairFinder:
    def __init__(
        ...
        self.pairing_shader = PairingConvolutionShader(
            device, preproced_view.texture.format
        )
        self.pairing_pipeline = self.pairing_shader.create_compute_pipeline()
    ...
    def calc_pairing_value(self, points_position: numpy.ndarray, points_rotation: numpy.ndarray) -> numpy.ndarray:
        ...
        wgpu_util.push_compute_pass(
            command_encoder, self.pairing_pipeline, pairing_binds,
            pairing_size
        )
...
if __name__ == "__main__":
    ...
    pair_finder = MarkerPairFinder(device, preproced_view, matched_view, fisheye_kernel_ndarray())
