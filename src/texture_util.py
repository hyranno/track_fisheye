...
def draw_texture_on_texture(
    ...
    sampler_filter = (
        ...
        if (linear_interp) else
        (wgpu.FilterMode.nearest, wgpu.SamplerBindingType.non_filtering)
    )
    sampler = device.create_sampler(min_filter=sampler_filter[0], mag_filter=sampler_filter[0])
    sample_type = sampler_filter[1]

    tex_shader = TextureShader(device, src.format, sample_type)
